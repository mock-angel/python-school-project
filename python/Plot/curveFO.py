...
class curveFvO():# Curve fitting via optimisation
    ...
    def loadSineWaveTest(self):
        ...
        #return
        from scipy.optimize import curve_fit;print "hello"
        #import matplotlib
...
#        print 
        ...
        param, param_cov = curve_fit(test, self.list_x, self.list_y) 
        self.param = param
    ...
    def function_sine_wave_fitting(self, x):
        #param = self.param
        #return (-2.* np.cos(np.pi) - (5.*np.cos(8*np.pi))/8.)*(np.pi/2)#5*np.sin(8*x) # + 5*np.sin(8*x)
#        x = 1.3
#        print x, (((2.*8.)+(5.))/(9.))* np.sin((1)*x)
#        x = 1.4
#        print x, (((2.*8.)+(5.))/(9.))* np.sin((1)*x)
#        x = 1.5
#        print x, (((2.*8.)+(5.))/(9.))* np.sin((1)*x)
...
#        wb = load_workbook('plotdata.xlsx')
#        ws = wb.active
#        x = 0
#        while x < 10:
#            x += 0.1
#            y = 2*np.sin((1)*x)+5*np.sin((8)*x)
#            ws.append([x, y] )
#        wb.save("plotdata.xlsx")
#        exit()
#        return 2* np.sin(1*x) + 5*np.sin(8*x)
        #return 5*np.sin(8*x)
#        return 2*(np.sin(1*x))# + 2*(np.sin(1.1*x))
        
        param = self.param
        return (param[0])*np.sin(param[1]*x)
    def function_sine_wave_fittingw(self, x):
        #param = self.param
        return (-2.* np.cos(np.pi) - (5.*np.cos(8*np.pi))/8.)*(np.pi/2)#5*np.sin(8*x) # + 5*np.sin(8*x)
    def getPoints(self):
        return self.list_xy
